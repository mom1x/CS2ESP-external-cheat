...
def esp(draw_list):
    try:
        ...
        if local_player <= 0 or local_player > 0x7FFFFFFFFFFFFFFF:
            return
        ...
        for i in range(64):
            ...
            if entity <= 0 or entity > 0x7FFFFFFFFFFFFFFF:
                continue

            # Находим адрес entity_controller
            addr1 = entity + ((8 * (i & 0x7FFF) >> 9) + 16)
            if addr1 <= 0 or addr1 > 0x7FFFFFFFFFFFFFFF: continue
            list_entry = pm.read_longlong(addr1)
            if not list_entry:
                continue

            addr2 = list_entry + (120) * (i & 0x1FF)
            if addr2 <= 0 or addr2 > 0x7FFFFFFFFFFFFFFF: continue
            entity_controller = pm.read_longlong(addr2)
            ...
            # Достаем сам pawn
            addr3 = entity + (0x8 * ((entity_controller_pawn & 0x7FFF) >> 9) + 16)
            if addr3 <= 0 or addr3 > 0x7FFFFFFFFFFFFFFF: continue
            list_entry = pm.read_longlong(addr3)
            if not list_entry:
                continue

            addr4 = list_entry + (120) * (entity_controller_pawn & 0x1FF)
            if addr4 <= 0 or addr4 > 0x7FFFFFFFFFFFFFFF: continue
            entity_pawn = pm.read_longlong(addr4)
            ...
            if game_scene <= 0 or game_scene > 0x7FFFFFFFFFFFFFFF: continue
            
            bone_matrix = pm.read_longlong(game_scene + m_modelState + 0x80)
            if bone_matrix <= 0 or bone_matrix > 0x7FFFFFFFFFFFFFFF: continue

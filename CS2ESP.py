...
def process_visuals_and_sync(draw_list):
    ...
    try:
        ...
        if not local_ctrl and local_idx_global != -1:
            try: local_ctrl = pm.read_longlong(list_entry + 112 * local_idx_global)
            except: pass
        ...
        if m_pObserverServices and m_hObserverTarget:
            try:
                ...
                if local_obs_services:
                    local_target = pm.read_uint(local_obs_services + m_hObserverTarget)
                    if local_target and local_target != 0xFFFFFFFF:
        ...
        if menu_open:
            ...
            if imgui.begin_tab_bar("CheatTabs"):
                if imgui.begin_tab_item("VISUALS")[0]:
                    imgui.spacing()
                    _, cfg_esp_enabled = imgui.checkbox("Master ESP Toggle (ALT)", cfg_esp_enabled)
                    imgui.separator()
